...
export default class Game
{
    ...
    detectNextPlayer()
    {
        // return this.currentPlayer.getNextThrowNumber(this.currentSet, this.currentLeg, this.currentTurn) > DartDefinition.maxThrowsPerTurn;
        return this.currentThrow+1 > DartDefinition.maxThrowsPerTurn;
    }
    ...
    {
        var result = $.ajax({
            url: DartDefinition.apiBasePath+'/dart/updatePlace/'+this.id+'/'+this.currentPlayer.id,
            type: 'PUT',
            async: false,
            headers: {
    ...
    _changeStatus(status = null)
    {
        if(status)
            var result = $.ajax({
                url: DartDefinition.apiBasePath+'/dart/'+this.id,
                type: 'PUT',
                async: false,
                headers: {
                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
